...
export class SimulationFormulaireComponent implements OnInit{
  ...
  @Output() formSubmitted = new EventEmitter<{ totalParticipants: number, numberOfDraws: number, drawType: string }>();
  ...
  ngOnInit(): void {
    ...
    this.simulationForm = this.formBuilder.group({
      totalParticipants: [null, [Validators.required, Validators.min(0), Validators.max(100)]],
      numberOfDraws: [{ value: null, disabled: true }, [Validators.required, Validators.min(1)]],
      drawType: ['auto', Validators.required]
    });
  ...
  TotalParticipantsChanges(): void {
    this.simulationForm.get('totalParticipants')?.valueChanges.subscribe(value => {
      ...
      if (this.simulationForm.get('totalParticipants')?.valid) {
        ...
        numberOfDrawsControl?.setValidators([Validators.required, Validators.min(1), Validators.max(value)]);
      } else {
