...
function classerTirages(tirages, tirageWin) {
  const tiragesTries = tirages.sort((tirageA, tirageB) => {
    ...
    if (comparisonA.numMatching !== comparisonB.numMatching) {
      ...
      return (
        scoreNonInclude(tirageA, tirageWin) -
        scoreNonInclude(tirageB, tirageWin)
      );
    }
  });
...
function scoreInclude(tirage, tirageWin) {
  const numMatching = tirage.numeros.filter((num) =>
    tirageWin.numeros.includes(num),
  ).length; // Correspondance des numéros
  const etoilesMatching = tirage.etoiles.filter((star) =>
    tirageWin.etoiles.includes(star),
  ).length; // Correspondance des étoiles
...
function scoreNonInclude(tirage, tirageWin) {
  const numNonMatching = tirage.numeros.filter(
    (num) => !tirageWin.numeros.includes(num),
  );
  const numNonMatchingWin = tirageWin.numeros.filter(
    (num) => !tirage.numeros.includes(num),
  );

  const etoileNonMatching = tirage.etoiles.filter(
    (star) => !tirageWin.etoiles.includes(star),
  );
  const etoileNonMatchingWin = tirageWin.etoiles.filter(
    (star) => !tirage.etoiles.includes(star),
  );
...
function classerPosition(tirages, tiragewin) {
  ...
  for (let index = 0; index < tirages.length; index++) {
    if (index === 0) {
      ...
      if (
        scoreNonInclude(tirages[index - 1], tiragewin) ===
        scoreNonInclude(tirages[index], tiragewin)
      ) {
