...
if (etoileGagnant) {
  ...
  const etoileGagnantArray = etoileGagnant.split(',').map(num => num.trim());
...
async function saveTirage(tirages, montantPartie, tirageGagnant, totalParticipants) {
  ...
  try {
    ...
    const insertTirageResult = await client.query(`
      ...
      VALUES ($1, $2, $3, $4, $5)
      RETURNING id;
    `, [now, tirageGagnant.numeros, tirageGagnant.etoiles, montantPartie, totalParticipants]);
    ...
    console.error('Erreur lors de l\'enregistrement des tirages et participations:', err);
    throw new Error('Erreur lors de l\'enregistrement des tirages et participations');
  } finally {
